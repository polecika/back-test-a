...
class HelicopterView
{
	...
	/*В доках "JSON formatting method" называется "get_format"
	возвращает заранее прописанное знамение "num" массивом.
	Внутри метода не используется. Зачем он здесь вообще?
	*/
	/**
	...
	/*Метод не вернет ошибку, если $method не равен "HVFeedback"
	Где видимость(public, private, protected, static)?
	Название метододов должно быть в едином стиле(в идеале, CamelCase)
	Зачем хранить обязательные поля в методе, так ещё и в строке, чтобы потом её резать? Тогда лучше создать константный массив
	*/
	...
	public function HVCountry(): array
	{
		...
		/*Нет проверки на пришедший неверный формат date_from и date_to*/
		$date_from = microTimeToTime($this->post['date_from']);
		$date_to = microTimeToTime($this->post['date_to']);

		/*Нет проверки на то, пришла ли переменная*/
	...
	public function getProductsCounterData($country_shortnames, $product_nicknames, $date_from, $date_to)
	{
		...
		____
		этот кусок можно было бы написать один раз, а  ifом склеить запрос
		*/
...
//        $this->post = json_decode('{"date_from":1654027200000,"date_to":1654545600000,"products":["SC-BL","BC"],"countries":null,"group_by":"platinum","aggregate":false}', true);
	...
	public function HVFeedbackType() : array {
		...
		/*проверка типов, должны прийти массивы */
	...
	public function HVOverallDates(): array
	{
		...
		/*понадобились только первые значения, а не все
		Значит в запрос нужно было дописать LIMIT 1 */
		$dates_object = [
	...
	private function getCounterByCountryIds($date_from, $date_to, $country_ids, $product_nicknames): array
	{
		...
		/*$country_ids может оказаться пустым массивом, и тогда последний AND в запросе даст ошибку*/
		foreach ($country_ids as $country_id) {
			...
			/*$product_ids тоже может оказаться пустым и запрос даст ошибку*/
		...
		foreach ($country_ids as $country_id) {
			/*$countryCounterById может быть пустым*/
	...
	private function countOtherCountryIds($other_country_ids)
	{
		...
		foreach ($other_country_ids as $country_id) {
			/*проверка на наличие ключа count*/
	...
	{
		/* в таблице country нет поля full_name*/
	...
	private function getReviewsDataByPeriod($date_from, $date_to, $country_id): array
	{
		...
		/*$this->post['products'] может не быть*/
	...
	private function getIntervalMatrix($date_from, $date_to, $interval = 3600)
	{
		...
		for ($i = 1; $i <= $countHoursBetweenDates; $i++) {
			/*не понятное название переменной $k*/
	...
	private function getProductIdsByCountryShortname($product_nicknames, $product_countries) {
		...
		if(!empty($product_nicknames)) {
			$sql .= " where bp.short_name IN ($product_nicknames)";
		}
		if(!empty($product_countries)) {
			$sql .= !empty($product_nicknames) ? ' AND ' : ' ';
			$sql .= "WHERE country.short_name IN ($product_countries)";
		}
		*/
	...
	{
		/*Снова много лишнего, дублируется текст запроса
		принцип см. метод getProductIdsByCountryShortname()
		*/
		if (empty($product_countries) && empty($product_nicknames)) {
			$sql = "SELECT `products`.`id` 
                    FROM business_products bp
